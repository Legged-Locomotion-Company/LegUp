...
class ConcurrentTrainingEnv(VecEnv):
    ...
    def __init__(self, num_environments: int, asset_path: str, asset_name: str):
        ...
        self.default_dof_pos = torch.Tensor(
            [0, -0.8, 1.6, 0, -0.8, 1.6, 0, -0.8, 1.6, 0, -0.8, 1.6]).to(self.device)
        ...
        self.cmd = torch.Tensor([1, 0, 0]).repeat(num_environments, 1).to(
            self.device)  # desired [x_vel, y_vel, ang_vel]
        self.gravity_vec = torch.Tensor(
            [0, 0, -1]).repeat(num_environments, 1).to(self.device)
        ...
        self.des_joint_pos_hist = HistoryBuffer(
            num_environments, self.dt, 0.02, 2, 12, self.device, fill=self.default_dof_pos)
        self.joint_pos_err_hist = HistoryBuffer(
            num_environments, self.dt, 0.02, 3, 12, self.device)
        self.joint_vel_hist = HistoryBuffer(
            num_environments, self.dt, 0.02, 3, 12, self.device)

        self.prev_obs = HistoryBuffer(
            num_environments, self.dt, self.dt, 5, 153, self.device)
    ...
    def make_reward(self, root_quat, root_ang_vel, joint_pos, joint_vel, des_jpos_t1, des_jpos_t2, command, root_lin_vel, foot_height, contact_probs, actions) -> torch.Tensor:
        ...
        r_q_dd = G.k_q_dd * \
            self.sq_norm(joint_vel - self.joint_vel_hist.get(0))

        r_s1 = G.k_s1 * self.sq_norm(actions - des_jpos_t1)

        r_s2 = G.k_s2 * self.sq_norm(actions - 2 * des_jpos_t1 + des_jpos_t2)

        r_base = G.k_base * (0.8 * torch.pow(root_lin_vel[:, 2], 2) + 0.2 * torch.abs(
            root_ang_vel[:, 0]) + 0.2 * torch.abs(root_ang_vel[:, 1]))
        ...
        # in the paper, they only sum 3 feet instead of 4?
        r_pos = r_v + r_w + torch.sum(r_air, dim=-1)
        r_neg = r_ori + r_t + r_q + r_q_d + r_q_dd + r_s1 + r_s2 + r_base + \
            torch.sum(r_slip + r_cl, dim=-
                      1)  # in the paper, they only sum 3 feet instead of 4?
        r_tot = r_pos + torch.exp(0.2 * r_neg)

        terms = torch.mean(torch.stack([r_tot, r_pos, r_neg, r_v, r_w, torch.sum(r_air, dim=-1), torch.sum(
            r_slip, dim=-1), torch.sum(r_cl, dim=-1), r_t, r_q, r_q_d, r_q_dd, r_s1, r_s2, r_base, r_ori]), dim=1)
        names = ['total', 'pos', 'neg', 'r_v', 'r_w', 'r_air', 'r_slip', 'r_cl',
                 'r_t', 'r_q', 'r_qdot', 'r_qddot', 'r_s1', 'r_s2', 'r_base', 'r_ori']
    ...
    def make_observation_and_reward(self, actions: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        contact_probs = self.env.get_contact_states(
        )[:, self.feet_idx]  # (num_envs, 4)

        reward, terms, names = None, None, None
        if actions is not None:
            reward, terms, names = self.make_reward(root_quat, root_ang_vel, joint_pos, joint_vel,
                                                    des_jpos_t1, des_jpos_t2, command, root_lin_vel, foot_height, contact_probs, actions)

        obs_list = [root_quat, root_ang_vel, joint_pos, joint_vel, des_jpos_t1, des_jpos_t2,
                    Q_err_hist, Q_vel_hist, relative_feet_pos, command, root_lin_vel, foot_height, contact_probs]
    ...
    def check_termination(self) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        """
        term = torch.any(self.env.get_contact_states()[
                         :, self.term_contacts], dim=-1)
    ...
    def step(self, actions: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, dict]:
        ...
        new_obs, reward, terms, names = self.make_observation_and_reward(
            actions)
