...
class GPURolloutBuffer(GPUBaseBuffer):
    ...
    def reset(self) -> None:
        ...
        self.actions = th.zeros(
            (self.buffer_size, self.n_envs, self.action_dim), dtype=th.float32, device=self.device)
    ...
    def _get_samples(self, batch_inds: th.Tensor, env: Optional[VecNormalize] = None) -> RolloutBufferSamples:
        data = (
            ...
            self.values[batch_inds].clone().flatten(),
            self.log_probs[batch_inds].clone().flatten(),
            self.advantages[batch_inds].clone().flatten(),
            self.returns[batch_inds].clone().flatten(),
        )
