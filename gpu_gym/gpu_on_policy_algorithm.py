...
from gpu_gym.gpu_base_alg import GPUBaseAlgorithm as BaseAlgorithm
from gpu_gym.gpu_buffers import GPURolloutBuffer as RolloutBuffer
...
OnPolicyAlgorithmSelf = TypeVar(
    "OnPolicyAlgorithmSelf", bound="GPUOnPolicyAlgorithm")


class GPUOnPolicyAlgorithm(BaseAlgorithm):
    """
    ...
    def _setup_model(self) -> None:
        ...
        buffer_cls = DictRolloutBuffer if isinstance(
            self.observation_space, gym.spaces.Dict) else RolloutBuffer
    ...
    def collect_rollouts(
        ...
        while n_steps < n_rollout_steps:
            ...
            if isinstance(self.action_space, gym.spaces.Box):
                torch_low = th.from_numpy(
                    self.action_space.low).to(self.device)
                torch_high = th.from_numpy(
                    self.action_space.high).to(self.device)
                clipped_actions = th.clamp(
                    actions, torch_low, torch_high)
