...
class GPUBaseAlgorithm(ABC):
    ...
    def __init__(
        ...
        if env is not None:
            ...
            if isinstance(self.action_space, gym.spaces.Box):
                assert th.all(
                    th.isfinite(
                        th.tensor([self.action_space.low, self.action_space.high]))
            ...
            if isinstance(env.observation_space, gym.spaces.Dict):
                ...
                for space in env.observation_space.spaces.values():
                    wrap_with_vectranspose = wrap_with_vectranspose or (
                        is_image_space(
                            space) and not is_image_space_channels_first(space)
                    )
            else:
    ...
    def _setup_learn(
        ...
        callback = self._init_callback(
            callback, eval_env, eval_freq, n_eval_episodes, log_path, progress_bar)
