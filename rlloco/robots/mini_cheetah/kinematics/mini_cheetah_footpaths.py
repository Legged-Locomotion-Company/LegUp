...
def walk_half_circle_line(q_vec, pos_phase_deltas, phase):
    ...
    Args:
        ...
        phase (torch.Tensor): a (NUM_ENVS) tensor with the scalar robot phase for each environment
    ...
    return use_gait_footpath(q_vec, pos_phase_deltas, phase, half_circle_line, phase_offsets)
...
def use_gait_footpath(q_vec, pos_phase_deltas, phase, path_func, phase_offsets):
    ...
    Args:
        ...
        pos_and_phase_deltas (torch.Tensor): a (NUM_ENVS x 12) vector giving (x,y,z) deltas from the current position along the preplanned path for the foot, stacked with a theta delta for each of the foot phases
        phase (torch.Tensor): a (NUM_ENVS) vector giving the gait phase for the robot in each of the envs
